...
async function cleanupInstrumentsFile() {
  return fs.unlink('instruments.csv');
}
...
async function getATMStraddle(underlying, exchange, nfoSymbol, strikeStepSize, skewThreshold) {
  ...
  console.log({ underlyingLTP, atmStrike });
  ...
  if (skew >= skewThreshold) {
    await delay(2 * 1000);
    return getATMStraddle(underlying, exchange, nfoSymbol, strikeStepSize, skewThreshold);
  }
...
export default async ({ instrument, lots, user, maxSkewPercent = 10, slmPercent = 50 }) => {
  ...
  const SKEW_THRESHOLD = maxSkewPercent;
