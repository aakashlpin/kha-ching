...
export default async (initialJobData) => {
  ...
  const lastOpenDate = getLastOpenDateSince(dayjs(getDateTimeInIST())).format(DATE_FORMAT);
  ...
  try {
    ...
    if (
      ...
      // onwards to take the next trade
      // one last check before we do that
      if (lastTrend) {
        // after 1 min get ST again
        await delay(ms(60));
        // and if ST is same as previous minutes' trend, only then punch trade
        // (there are certain situations where ST toggles back to previous trend and gives a false signal on the 5 min candle)
        const nextMinSTRes = await fetchSuperTrend({
          ...supertrendProps,
          to_date: dayjs(supertrendProps.to_date, DATE_TIME_FORMAT)
            .add(1, 'minute')
            .format(DATE_TIME_FORMAT),
          latest_only: true
        });

        const [nextMinTrendData] = nextMinSTRes.slice(-1);
        // if ST has toggled again from last one min, ignore the trend change
        if (nextMinTrendData.STX_10_3 !== currentTrendData.STX_10_3) {
          const error = `[directionalOptionSelling] new trend ("${currentTrendData.STX_10_3}") couldn't sustain the next minute trend ("${nextMinTrendData.STX_10_3}"). Will keep trying!`;
          console.log(error);
          return Promise.reject(error);
        }
      }
    ...
    if (maxTrades > 1) {
      if (getTimeLeftInMarketClosingMs() < 25 * 60 * 1000) {
        return `🟢 [directionalOptionSelling] Terminating DOS trade. ${maxTrades} attempts left but less than 25 mins in market closing.`;
      }
