...
async function punchOrders (initialJobData, superTrend, instrumentsRawData) {
    ...
    if (hedgeTradingSymbol) {
      ...
      if (!MOCK_ORDERS) {
        ...
        if (successful) {
          ...
          throw new Error('hedge order unsuccessful after several retries! terminating dos')
        }
      } else {
        hedgeOrderResponse = mockOrderResponse[0]
      }
    }
  }
  ...
  if (!MOCK_ORDERS) {
    ...
    if (successful) {
      ...
      throw new Error('DOS order unsuccessful after several retries! terminating dos')
    }
  } else {
    rawKiteOrderResponse = mockOrderResponse[0]
  }
