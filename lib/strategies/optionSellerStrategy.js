...
export default async initialJobData => {
  try {
    ...
    try {
      ...
      const callOptionStrike =
        Math.round(callOptionPrice / strikeStepSize) * strikeStepSize
      const putOptionStrike =
        Math.round(putOptionPrice / strikeStepSize) * strikeStepSize
