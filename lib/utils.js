...
import { EXIT_STRATEGIES, getMisOrderLastSquareOffTime, STRATEGIES } from './constants';
...
export const getNearestCandleTime = (intervalMs) => {
  ...
  const date = dayjs(); //or use any other date
  const roundedToNearest5thMin = dayjs(Math.floor(date.valueOf() / intervalMs) * intervalMs);
...
export const getNextNthMinute = (intervalMs, initDate = new Date()) => {
  // ref: https://stackoverflow.com/a/10789415/721084
  const rounded = new Date(Math.ceil(dayjs(initDate).valueOf() / intervalMs) * intervalMs);
  return rounded;
};
