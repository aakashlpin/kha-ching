...
import { ensureIST } from './browserUtils';
...
export const getMisOrderLastSquareOffTime = () => {
  const dateInIST = getDateTimeInIST();
  console.log('[getMisOrderLastSquareOffTime] dateInIST', dayjs(dateInIST).format());
  const time = dayjs(dateInIST).set('hour', 15).set('minutes', 24).set('seconds', 0).format();
  console.log('[getMisOrderLastSquareOffTime] time', dayjs(time).format());
  return time;
};
...
export const getTimeLeftInMarketClosingMs = () =>
  process.env.NEXT_PUBLIC_APP_URL.includes('localhost:')
    ...
    : dayjs(getMisOrderLastSquareOffTime()).diff(dayjs(getDateTimeInIST()));
...
export const getDateTimeInIST = () => {
  const timeInUTC = new Date();
  // this will lose the TZ info
  const timeInIST = new Date(timeInUTC.toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
  // plug the TZ info
  const dateTimeWithIST_TZ = ensureIST(timeInIST);
  return dateTimeWithIST_TZ;
};
