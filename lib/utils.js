...
export const remoteOrderSuccessEnsurer = async (args) => {
  ...
  console.log({ attemptCount, retryAttempts })
  if (attemptCount > 0) {
  ...
  try {
    ...
    try {
      const successful = await finiteStateChecker(isOrderInUltimateStatePr, orderStatusCheckTimeout)
      if (successful) {
        return true
      }
    ...
    if (
      ...
      try {
        ...
        const isMatchedOrderInUltimateStatePr = orderStateChecker(matchedOrder.order_id)
        try {
