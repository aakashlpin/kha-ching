...
export const getMisOrderLastSquareOffTime = () =>
  dayjs(getDateTimeInIST()).set('hour', 15).set('minutes', 24).set('seconds', 0).format();
...
export const getDateTimeInIST = () => {
  ...
  // this will lose the TZ info again
  const timeInIST = new Date(timeInUTC.toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
  // plug the TZ info
  console.log({ timeInIST: dayjs(timeInIST).format() });
  const dateTimeWithIST_TZ = ensureIST(timeInIST);
  console.log({ dateTimeWithIST_TZ: dayjs(dateTimeWithIST_TZ).format() });
