...
export function getPercentageChange(price1, price2) {
  return Math.round((Math.abs(price1 - price2) / Math.min(price1, price2)) * 100);
}
...
export function closest(needle, haystack, haystackKey) {
  // https://www.gavsblog.com/blog/find-closest-number-in-array-javascript
  return haystack.reduce((a, b) => {
    const compareA = haystackKey ? a[haystackKey] : a;
    const compareB = haystackKey ? b[haystackKey] : b;
    let aDiff = Math.abs(compareA - needle);
    let bDiff = Math.abs(compareB - needle);
...
export const getTradingSymbolsByOptionPrice = async ({
  ...
  try {
    ...
    const closestStrike = closest(price, formattedPrices, 'last_price');
