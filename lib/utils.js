...
const finiteStateChecker = (infinitePr, checkDurationMs) => {
  return infinitePr.timeout(checkDurationMs).catch(e => {
    console.log('finiteStateChecker', e)
    infinitePr.cancel()
  })
}
...
export const remoteOrderSuccessEnsurer = async (args) => {
    ...
    return new Promise((resolve, reject, onCancel) => {
      ...
      const fn = async () => {
        ...
        try {
          ...
          // console.log(e)
          if (e?.status === 'error' && e?.error_type === 'GeneralException' && e?.message === "Couldn't find that `order_id`.") {
            throw new Error(kite.STATUS_REJECTED)
          }
  ...
  try {
    ...
    try {
      const successful = await finiteStateChecker(isOrderInUltimateStatePr, ms(2))
      if (successful) {
        return true
      }
