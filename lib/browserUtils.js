...
export const handleCreateJob = async (jobProps) => {
  const {
    lots,
    slmPercent,
    runNow,
  ...
  const apiProps = {
    instruments: Object.keys(instruments).filter((key) => instruments[key]),
    lots: Number(lots),
  ...
  if (!apiProps.instruments.length) {
    return Promise.reject('Please select an instrument');
  }

  return axios.post('/api/create_job', apiProps);
};
