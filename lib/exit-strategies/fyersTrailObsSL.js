...
import { getNearestCandleTime, ms } from '../utils';
...
export default async ({
  ...
  try {
    ...
    const currentClose = MOCK_ORDERS ? 200 : await getPreviousMinClose(instrumentToken);
    const profitPercent = ((currentClose - entryPrice) / entryPrice) * 100;
    const trailingPercent = profitPercent / 2;
    const revisedSL = Math.round((initialSLPrice * trailingPercent) / 100 + initialSLPrice);
    ...
    return Promise.reject(`[fyersTrailObsSL] will retry!`);
  } catch (e) {
    console.log('[fyersTrailObsSL] global caught error', e);
    return Promise.reject('[fyersTrailObsSL] global caught error. Will retry!');
  }
};
