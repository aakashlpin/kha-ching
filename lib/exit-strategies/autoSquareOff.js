...
async function doDeletePendingOrders (orders, kite) {
  ...
  if (!openOrders.length) {
    logObject('[doDeletePendingOrders] no trigger pending orders.')
  }
  ...
  if (!openOrdersForPositions.length) {
    logObject('[doDeletePendingOrders] no openOrdersForPositions')
  }
...
async function autoSquareOffStrat ({ rawKiteOrdersResponse, deletePendingOrders, initialJobData }) {
  ...
  if (!completedOrders) {
    ...
    throw new Error('Initial order not completed yet!? Auto Square Off failed!')
  }
