...
const optionSellerSLWatcher = async ({ slOrderAckId, entryPrice, slTriggerPrice, watchForOrderState, initialJobData, attemptCount, maxAttempts }) => {
  try {
    ...
    const { order_id: reEntryOrderAckId } = await kite.placeOrder(kite.VARIETY_REGULAR, reEntryOrder)
