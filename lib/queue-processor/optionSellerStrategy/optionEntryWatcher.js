...
import { getIndexInstruments, getInstrumentPrice, getTradingSymbolsByOptionPrice, syncGetKiteInstance } from '../../utils'

import console from '../../logging'
// import { addToNextQueue, WATCHER_Q_NAME } from '../queue'

const optionSellerEntryWatcher = async ({ limitOrderAckId, entryPrice, slTriggerPrice, watchForOrderState, initialJobData, addHedge }) => {
  try {
    ...
    const { order_id: slOrderAckId } = await kite.placeOrder(kite.VARIETY_REGULAR, slOrder)
    ...
    const {
      ...
      instrumentType: completedOrder.tradingsymbol.substr(completedOrder.tradingsymbol.length - 2, completedOrder.tradingsymbol.length - 1),
      user
    })
    ...
    const { order_id: hedgeOrderAckId } = await kite.placeOrder(kite.VARIETY_REGULAR, hedgeOrder)
