...
export function useUser({ redirectTo = false, redirectIfFound = false } = {}) {
    ...
    return { user: typeof user === "object" ? { ...user, isClubMember: user.isClubMember || isClubMember } : user, mutateUser };
}
...
export function usePlans() {
    ...
    return {
        plans, mutatePlans, isLoading, isError, rawPlans: data
    }
}
...
export function usePnL(tradesInfo) {
    ...
    useEffect(() => {
        ...
        const updatePnLForAllTrades = () => {
            setIsLoading(true);
            Promise.all((tradesInfo.filter(t => t.orderTag).map(trade => axios.get(`/api/pnl?order_tag=${trade.orderTag}`))))
