...
export default withSession(async (req, res) => {
  ...
  if (req.method === 'PUT') {
    ...
    if (order) {
      await axios.delete(`${withoutFwdSlash(DATABASE_HOST_URL!)}/odr_${DATABASE_USER_KEY!}/${order._collection!}/${order._id as string}`, SIGNALX_AXIOS_DB_AUTH)
    }
    ...
    const { data: updatedRes } = await axios.post(
      `${withoutFwdSlash(DATABASE_HOST_URL!)}/odr_${DATABASE_USER_KEY!}/${orderTag as string}`,
      {
        ...newDbOrder,
        tag: orderTag
      },
  ...
  const dbOrdersUrls = allTags.map(
    (tag) => `${withoutFwdSlash(DATABASE_HOST_URL!)}/odr_${DATABASE_USER_KEY!}?limit=100&q=tag:${tag as string}`
  )
  // console.log({ dbOrdersUrl });
  const responses = await Promise.all(dbOrdersUrls.map(async (dbOrdersUrl) => axios(dbOrdersUrl)))
  const dbOrders = responses.reduce((accum: [], response: { data: []}) => [...accum, ...response.data], [])
